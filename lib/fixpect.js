...
function ensureAfterBlockIsRegistered() {
    ...
    after(function () {
        ...
        for (const fileName of Object.keys(fixesByFileName)) {
            var fixResult = SourceCodeFixer.applyFixes(getSourceCode(fileName).text, fixesByFileName[fileName]);
            if (fixResult.fixed) {
                ...
                .then(results => {
                    results.forEach((resultEntry, i) => {
                        if (resultEntry[1]) {
                            ...
                            isUnderVersionControlByFileName[fileNames[i]] = resultEntry[0] === 0;
                        }
                    });
                ...
                .then(([ , err, dirtyFileNames]) => {
                    ...
                    setImmediate(function () {
                        ...
                        if (/^(?:1|true|on|yes)$/i.test(process.env.FIXPECT)) {
                            ...
                            console.log('rerun the command with FIXPECT=yes to overwrite your test files with fixed versions');
                            replayAndRestoreProcessExit();
                        }
                    });
                });
        ...
        expect.hook(function (next) {
            return function fixpect(subject, ...rest) {
                ...
                function onError(error) {
                    const fixes = typeof error.fixes === 'function' ? error.fixes() : error.fixes;
                    if (fixes) {
