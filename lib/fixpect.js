  ...
  after(function() {
      ...
      new Traverser().traverse(sourceCode.ast, {
        enter(node, parent) {
          ...
          if (
            ...
            node.loc.start.line === topLevelError.lineNumber &&
            node.loc.start.column + 1 === topLevelError.columnNumber &&
            ...
            Object.keys(topLevelError.fixes).forEach(argNumber => {
              (fixesByFileName[fileName] =
                ...
                fix: ruleFixer.replaceText(
                  node.arguments[parseInt(argNumber, 10) + 2],
                  stringify(
                    topLevelError.fixes[argNumber],
                    indentationWidth,
                    expectForRendering
                  ).replace(/\n^/gm, `\n${getNodeIndent(node)}`)
                )
    ...
    if (fileNames.length > 0) {
      ...
      return Promise.map(
        ...
        .then(results => {
          results.forEach((resultEntry, i) => {
            if (resultEntry[1]) {
              firstError = firstError || resultEntry[1];
            } else {
              isUnderVersionControlByFileName[fileNames[i]] =
                resultEntry[0] === 0;
            }
          });
          return run('git', ['diff-index', 'HEAD', '--name-only'], {
            bufferLines: true
          });
        })
        .then(([, err, dirtyFileNames]) => {
          ...
          setImmediate(function() {
            console.log(
              `fixpect was able to patch up ${numFixedExpects} expect call${
                numFixedExpects === 1 ? '' : 's'
            ...
            if (/^(?:1|true|on|yes)$/i.test(process.env.FIXPECT)) {
              if (process.stdout.isTTY) {
                ...
                inquirer
                  ...
                  .then(answers =>
                    Promise.map(
                      ...
                      fileName =>
                        fs.writeFileAsync(
                          fileName,
                          fixedSourceTextByFileName[fileName]
                        ),
                      { concurrency: 5 }
                    )
                  )
                  .then(results => {
                    console.log(
                      `fixpect: Wrote ${results.length} file${
                        results.length === 1 ? '' : 's'
                      }`
                    );
                ...
                return Promise.map(
                  ...
                  { concurrency: 5 }
                ).then(replayAndRestoreProcessExit);
              }
            } else {
              console.log(
                'rerun the command with FIXPECT=yes to overwrite your test files with fixed versions'
              );
              replayAndRestoreProcessExit();
            }
          });
    ...
    expect.hook(function(next) {
      return function fixpect(subject, ...rest) {
        ...
        function onError(error) {
          const fixes =
            typeof error.fixes === 'function' ? error.fixes() : error.fixes;
          if (fixes) {
